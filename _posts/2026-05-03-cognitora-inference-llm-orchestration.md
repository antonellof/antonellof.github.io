...
A single H100 with [vLLM](https://github.com/vllm-project/vllm) serves Llama 3 8B at strong throughput. A single node with [SGLang](https://github.com/sgl-project/sglang) handles structured generation well. [TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM) wrings every last token-per-second out of an NVL72. None of those are a *cluster*. The moment the workload outgrows one box, or the prefill phase wants different hardware than the decode phase, or a hot prefix shows up on the wrong replica, you are back to writing a routing layer, a KV-cache layer, and a deployment layer yourself.
...
*Updated September 2026: Cognitora is now at [v0.9.1](https://github.com/antonellof/cognitora-inference/releases/tag/v0.9.1). See [Cognitora v0.9.1: gossip discovery, live KV prefix, and a native engine](https://www.fratepietro.com/2026/cognitora-inference-update-v0.9/) for what shipped since v0.3.0 (gossip, carbon-aware admission, fleet dashboard, CognitoraConnector preview).*
