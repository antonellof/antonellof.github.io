---
layout: post
title: "Frink: a Rust GGUF engine, measured against llama.cpp"
date: 2026-08-05
categories: [Projects]
tags: [Rust, AI, LLM, Local Inference, Performance]
excerpt: "I built Frink, a pure-Rust GGUF inference engine. Not to replace llama.cpp, but to get MoE expert residency right, and every speed claim is pinned against llama.cpp on the same machine."
---

<img src="/assets/images/frink/frink-logo.webp" alt="Frink" width="380" />

*Written August 2026, at v0.4. Frink has moved a long way since, so the sections below carry update notes where the numbers changed.*

I built [Frink](https://github.com/antonellof/frink) over the last couple of weeks. It is a pure-Rust inference engine for GGUF models: dense and MoE, on CPU, Apple Metal, or CUDA. No bindings to llama.cpp. No wrapper around ggml.

## Where Frink is now

Many releases later, the [current one](https://github.com/antonellof/frink/releases/latest) is published on [crates.io](https://crates.io/crates/frink-inference). The short version:
...
- One binary does everything: completions, `frink serve` for the OpenAI and Anthropic compatible API, `frink download`, `frink bench`, `frink verify`. The only build flag you need is your GPU.
- `frink download` fetches a model with `hf download`'s exact syntax, so a command copied off a model card runs unchanged. No Python.
- Twelve real checkpoints answer correctly on Metal, two of them MoE.
- A web UI, Frink Studio, running as its own app against the same public API any other client uses.
- Published as `frink-inference`, with every layer available on its own if you want the GGUF reader and nothing else.

The middle of that stretch has its own post: [Frink on Metal](/2026/frink-metal-parity-llama-cpp/).
...
One binary with flags close to llama.cpp (`-m`, `-p`, `-n`, `-ngl`, `--ctk`, …). `frink serve` starts the HTTP server from the same executable, and `frink-server` still ships on its own for anyone who prefers two.
...
frink download bartowski/Llama-3.2-1B-Instruct-GGUF \
  Llama-3.2-1B-Instruct-Q4_K_M.gguf --local-dir models

# Frink wraps the prompt in the checkpoint's own chat template.
frink -m models/Llama-3.2-1B-Instruct-Q4_K_M.gguf \
  -p "What is 2+2?" -n 64 --temp 0 -dev metal -ngl all

# Serve on 127.0.0.1:8383 and point any OpenAI client at /v1.
frink serve -m models/Llama-3.2-1B-Instruct-Q4_K_M.gguf -dev metal -ngl all
```

The [README](https://github.com/antonellof/frink) lists known-good GGUFs to start from. Prefer `Q4_K_M` day to day, `Q8_0` for tiny tests. On Metal, `--ctk q8_0` shrinks the KV cache if you need longer context.
...
I do not trust speed claims without a method. `frink bench` runs the same `pp512` prefill and `tg128` decode workloads as `llama-bench`, on the same Mac (M2 Pro), the same GGUF and the same backend. Every run writes a JSON file of raw timings, those files are checked into the repo, and [`benchmarks/RESULTS.md`](https://github.com/antonellof/frink/blob/main/benchmarks/RESULTS.md) is generated from them rather than typed by hand. Gap = `llama / frink`. Under 1.0 means Frink is faster.

Engine decode on Metal, M2 Pro, measured at v0.12.0 in a single session:

| Model | tg128 Frink | tg128 llama.cpp | Gap |
|---|---|---|---|
...
CPU is the whole remaining gap, and Frink loses across it: all 16 comparable rows land between 1.41× and 5.06×. The cause is measured. Rayon forks and joins per operation while llama.cpp runs a persistent thread pool, and Frink is ahead at a single thread on Mistral-7B, so it scales badly rather than computing slowly. A replacement pool is written and waiting on a clean benchmark window.

Re-run it yourself:

```bash
frink bench --suite --fit-host --skip-missing
```
...
- **CUDA was quietly broken.** `--features cuda` on the CLI did not enable CUDA in `frink-core`. Prefill never hit the GPU.
...
From the [roadmap](https://github.com/antonellof/frink/blob/main/docs/ROADMAP.md):
...
If you want to point an IDE or agent at the local server, there is a short cookbook: [`docs/AGENTS_COOKBOOK.md`](https://github.com/antonellof/frink/blob/main/docs/AGENTS_COOKBOOK.md).
...
Frink is Apache-2.0. Stars are nice. PRs and failed pins are more useful. If you try it and something is slow or wrong, open an issue with the GGUF name, backend, and the `frink bench` output.
...
- [docs/MODELS.md](https://github.com/antonellof/frink/blob/main/docs/MODELS.md)
- [docs/THIRD_PARTY_NOTICES.md](https://github.com/antonellof/frink/blob/main/docs/THIRD_PARTY_NOTICES.md)
